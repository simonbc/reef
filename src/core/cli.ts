...
import { runAgentOnce, type AgentEvent, type ChatTurn } from "./agent";
import { runCliHarness } from "./cli-harness";
import {
  formatContentList,
  formatContentRead,
  listContent,
...
import { createHarnessApp } from "./harness";
import { openTarget, resolveOpenTarget } from "./open";
...
export async function runReefCli(args: string[]): Promise<void> {
  ...
  await runPrompt(args.join(" ").trim());
}
